...
void InitCmdTable(CmdTable* cmd_table) {
  ...
  //// DumpCmd
  std::unique_ptr<Cmd> dumpptr = std::make_unique<DumpCmd>(
      kCmdNameDump, 2,
      kCmdFlagsRead | kCmdFlagsKv | kCmdFlagsDoThroughDB | kCmdFlagsUpdateCache | kCmdFlagsReadCache | kCmdFlagsSlow);
  ...
  ////HExpire
  std::unique_ptr<Cmd> hexpireptr =
      std::make_unique<HExpireCmd>(kCmdNameHExpire, -4, kCmdFlagsWrite | kCmdFlagsHash | kCmdFlagsFast);
...
/*
这个  Cmd::DoBinlog  函数的主要功能是处理写入操作的二进制日志（binlog）。
Cmd::DoBinlog
函数的主要作用是确保在执行写操作时，如果配置允许，则将操作记录到二进制日志中。
函数首先检查命令的有效性和类型，然后尝试获取网络连接和响应指针，确保它们有效。
如果任何步骤失败，则记录相应的警告并设置错误结果。
如果一切顺利，命令将被提议写入二进制日志，任何写入失败都会被记录并反馈错误信息。
